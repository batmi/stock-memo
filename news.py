...
_cache = TTLCache(CACHE_TTL, CACHE_MAX, name='news')
...
def fetch_one(stock, force_refresh=False):
    ...
    if news_list is None:
        ...
        #    (allow_stale — 캐시는 보관만 하고, 낡은 값을 쓸지는 여기서 정한다)
        return _cache.get(stock, default=[], allow_stale=True)

    _cache.put(stock, news_list)
    return news_list

...
│
│  ── 데이터 (코드가 아님 — 갱신에 배포가 필요 없다) ──────────────
├── data/
│   └── krx_holidays.json  # KRX 휴장일 (매년 연초 갱신)
│
...
**6. 매년/수시로 갱신되는 값은 코드가 아니라 `data/` 에 둔다.**
KRX 휴장일이 그렇습니다. 소스에 박혀 있으면 갱신이 곧 코드 수정 + 재배포가 되고,
그래서 미뤄지다 잊힙니다. 잊히면 **조용히 틀립니다** — 공휴일을 정규장으로 판정해
쉬는 날 내내 외부 API 를 두드리고 화면은 '장중'으로 표시됩니다.
`data/krx_holidays.json` 을 고치면 **재시작 없이** 1분 안에 반영되며(`prices` 가
파일 mtime 을 확인 간격을 두고 감시합니다), 마지막 등록 연도를 넘긴 날에는
하루 1회 경고 로그가 남습니다.

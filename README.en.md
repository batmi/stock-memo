...
*   **Production Environment (HTTPS)**: While it runs stably on a local network via `waitress`, integrating a reverse proxy (e.g., Nginx) to apply HTTPS (SSL) certificates is highly recommended for proper web publishing. When serving over HTTPS, set `SESSION_COOKIE_SECURE=1` so the session cookie never travels over plaintext. Behind a reverse proxy, also set `TRUSTED_PROXY_COUNT=1` (the number of proxies in front). Without it every client appears as the proxy IP, so five failed logins by anyone lock everyone out. Do not set it without a proxy — clients could then spoof `X-Forwarded-For` to bypass IP limits.
...
│   │   └── jobs.py         # Background threads (auto backup, regular/NXT close caching)
│   │

        ...
        tr.innerHTML = `
            ...
            <td data-label="관리" style="padding: 10px; text-align: right; white-space: nowrap;">
                <div style="display: inline-flex; gap: 4px; align-items: center; justify-content: flex-end;">
                    <button onclick="resetUserPassword('${adminUser.username}')" style="background: transparent; border: 1px solid var(--warning-color); color: var(--warning-color); padding: 0 8px; border-radius: 4px; font-size: 11px; cursor: pointer; width: auto; height: 26px; box-sizing: border-box; display: inline-flex; align-items: center; justify-content: center; margin: 0; box-shadow: none; transition: opacity 0.2s; white-space: nowrap;" onmouseover="this.style.opacity='0.8'" onmouseout="this.style.opacity='1'">비번 초기화</button>
                </div>
            </td>
        `;
    ...
    if (sortedData.length > 0) {
        sortedData.forEach(u => {
            ...
            tr.innerHTML = `
                ...
                <td data-label="관리" style="padding: 10px; text-align: right; white-space: nowrap;">
                    <div style="display: inline-flex; gap: 4px; align-items: center; justify-content: flex-end;">
                        <button onclick="toggleUserAllow('${escapeJsInAttr(u.username)}')" style="background: transparent; border: 1px solid ${allowBtnBg}; color: ${allowBtnBg}; padding: 0 8px; border-radius: 4px; font-size: 11px; cursor: pointer; width: auto; height: 26px; box-sizing: border-box; display: inline-flex; align-items: center; justify-content: center; margin: 0; box-shadow: none; transition: opacity 0.2s; white-space: nowrap;" onmouseover="this.style.opacity='0.8'" onmouseout="this.style.opacity='1'">${allowBtnText}</button>
                        <button onclick="resetUserPassword('${escapeJsInAttr(u.username)}')" style="background: transparent; border: 1px solid var(--warning-color); color: var(--warning-color); padding: 0 8px; border-radius: 4px; font-size: 11px; cursor: pointer; width: auto; height: 26px; box-sizing: border-box; display: inline-flex; align-items: center; justify-content: center; margin: 0; box-shadow: none; transition: opacity 0.2s; white-space: nowrap;" onmouseover="this.style.opacity='0.8'" onmouseout="this.style.opacity='1'">비번 초기화</button>
                        ${hasReset ? `<button onclick="dismissResetRequest('${escapeJsInAttr(u.username)}')" title="비밀번호를 바꾸지 않고 요청만 내립니다" style="background: transparent; border: 1px solid var(--neutral-color); color: var(--neutral-color); padding: 0 8px; border-radius: 4px; font-size: 11px; cursor: pointer; width: auto; height: 26px; box-sizing: border-box; display: inline-flex; align-items: center; justify-content: center; margin: 0; box-shadow: none; transition: opacity 0.2s; white-space: nowrap;" onmouseover="this.style.opacity='0.8'" onmouseout="this.style.opacity='1'">요청 해제</button>` : ''}
                        <button onclick="deleteUserAccount('${escapeJsInAttr(u.username)}')" style="background: transparent; border: 1px solid var(--danger-color); color: var(--danger-color); padding: 0 8px; border-radius: 4px; font-size: 11px; cursor: pointer; width: auto; height: 26px; box-sizing: border-box; display: inline-flex; align-items: center; justify-content: center; margin: 0; box-shadow: none; transition: opacity 0.2s; white-space: nowrap;" onmouseover="this.style.opacity='0.8'" onmouseout="this.style.opacity='1'">삭제</button>
                    </div>
                </td>
            `;
            adminUserList.appendChild(tr);

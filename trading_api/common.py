...
#
# 명령은 **한 번만 내려보낸다**(at-most-once, _take_pending_command 참고). 반복 전달은
# 서버 데이터로 보면 멱등하지만 운용자의 의도로 보면 멱등하지 않다 — 두 실행 사이에
# 일부러 지운 기록이 되살아난다. 전달만 되고 ack 가 오지 않으면 만료될 때까지 '처리 중'
# 으로 남았다가 '미처리'로 바뀐다. 운용자는 그것을 보고 다시 누르면 된다.

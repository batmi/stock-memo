...
2. **멱등**: brokerExecutionId 에 UNIQUE 제약을 걸고 INSERT OR IGNORE 로 경합을 막는다.
   ...
   **다만 갱신되지는 않는다** — 기존 기록을 그대로 돌려줄 뿐 값을 덮지 않는다. 뒤늦게
   확정된 체결 수량·실현손익을 고치는 길은 PATCH 하나뿐이다(POST 로는 조용히 무시된다).
3. **모의 표시**: isSimulated 는 기록에 표시만 남기는 컬럼이다. 예전에는 이 값으로 조회·합계에서
   빼기도 했지만 지금은 **아무 것도 가르지 않는다** — 합계·통계에서 무엇을 뺄지는 계좌 설정
   (exclude_from_stats) 하나가 정한다. 가상투자는 웹저널 **계정 자체를 따로 쓰므로** 실거래와
   섞이지 않는다. 다만 last-sync·목록 조회는 isSimulated 를 명시적으로 준 경우 걸러 준다 —
   클라이언트가 백필 기준점을 그 축으로 좁히기 때문이다.
